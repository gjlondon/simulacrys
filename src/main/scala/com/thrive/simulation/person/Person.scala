...
import com.thrive.simulation.actions.CommonerActions._
import com.thrive.simulation.actions.Farm.addProduceToInventory
import com.thrive.simulation.actions.TransitionHealth.transitionHealth
import com.thrive.simulation.actions._
import com.github.nscala_time.time.Imports._
import com.thrive.simulation.constants.Constants.ENERGY_PER_KILO_OF_FAT
import com.thrive.simulation.demographic._
import com.thrive.simulation.entity.Entity
import com.thrive.simulation.inventory.FoodInventory
import com.thrive.simulation.location.Location
import com.thrive.simulation.meal.Meal
import com.thrive.simulation.message.MailboxTypes.{Inbox, Mailbox, Outbox}
import com.thrive.simulation.message._
import org.joda.time.DateTime
import com.thrive.simulation.person.Handlers.{CommonerReplyHandlers, emptyReplyHandler}
import com.thrive.simulation.resource.Calorie.calorie
import com.thrive.simulation.resource.{Old => _, _}
import squants.energy.Energy
import squants.mass._
import squants.motion.Distance
import squants.space.Centimeters
import com.thrive.simulation.status._
...
import com.thrive.simulation.actions.LocalConfig.DEBUG
...
import com.thrive.simulation.person.healthCalculations.{calcBodyMassIndex, calcWeightStatus}
...
case class Commoner(name: String,
  ...
  def update(time: DateTime, location: Location): Commoner =  {
    ...
    // if com.thrive.simulation.person is not incapacitated, allow a voluntary action
    // by assumption, no action is allowed to take less than the length of a single tick
    // so it's safe to assume that in a given tick, a com.thrive.simulation.person will take at most one action
  ...
  def act(time: DateTime, location: Location, entity: Commoner): (Commoner, Outbox) =  {
    ...
    entity.currentActivity match {
      ...
      case Idle =>
        // an com.thrive.simulation.entity can initiate an action, which will consume at least one tick, and
  ...
  def getOtherEntitiesRequired(action: PersonAction,
    ...
    action match {
      case com.thrive.simulation.actions.Farm =>
        val farmToUse = location.facilities.collectFirst {
          case f: com.thrive.simulation.facility.Farm if f.isAvailable => f
        }
        farmToUse match {
          case None => None
          case Some(farm) =>
  ...
  private def doEat(entity: Commoner): (Commoner, Mailbox) = {
    ...
    meal match {
      case None =>
        if (DEBUG) {
          println(s"No com.thrive.simulation.meal could be constructed from ${entity.inventory}")
        }
        (entity, Mailbox.empty)
      case Some(eatenMeal) =>
        ...
        if (DEBUG) println(f"Yum com.thrive.simulation.meal for ${entity.name} ${eatenMeal.kilocalories}%1.3f new fat $fatGained to reach $newBodyFat." +
          f"Remaining com.thrive.simulation.inventory has ${newInventory.totalAvailableCalories}%1.3f")
  ...
  private def doFarm(entity: Commoner): (Commoner, Mailbox) = {
    ...
    if (DEBUG) {
      ...
      println(s"old com.thrive.simulation.inventory ${entity.name} ${entity.inventory.contents.get(cropToFarm)}; new com.thrive.simulation.inventory ${newInventory.contents.get(cropToFarm)}")
    }
