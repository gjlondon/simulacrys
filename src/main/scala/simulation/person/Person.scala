...
import simulation.actions.CommonerActions._
import simulation.actions.Farm.addProduceToInventory
import simulation.actions.TransitionHealth.transitionHealth
import simulation.actions._
import com.github.nscala_time.time.Imports._
import simulation.constants.Constants.ENERGY_PER_KILO_OF_FAT
import simulation.demographic._
import simulation.entity.Entity
import simulation.inventory.FoodInventory
import simulation.location.Location
import simulation.meal.Meal
import simulation.message.MailboxTypes.{Inbox, Mailbox, Outbox}
import simulation.message._
import org.joda.time.DateTime
import simulation.ActionCandidates
import simulation.person.Handlers.{CommonerReplyHandlers, emptyReplyHandler}
import simulation.resource.Calorie.calorie
import simulation.resource.{Old => _, _}
...
import simulation.actions.LocalConfig.DEBUG
...
import simulation.person.healthCalculations.{calcBodyMassIndex, calcWeightStatus}
...
sealed trait Person extends Entity {
  ...
  val foodEnergyRequired: Energy = {
    val required: Int = (ageBracket: AgeBracket, gender) match {
      ...
      case (simulation.demographic.Old, Female) => 1800
      case (simulation.demographic.Old, Male) => 1900
    }
...
case class Commoner(name: String,
  ...
  def update(time: DateTime, location: Location): Commoner =  {
    ...
    // interaction with another simulation.entity
    ...
    // if simulation.person is not incapacitated, allow a voluntary action
    // by assumption, no action is allowed to take less than the length of a single tick
    // so it's safe to assume that in a given tick, a simulation.person will take at most one action
  ...
  def act(time: DateTime, location: Location, entity: Commoner): (Commoner, Outbox) =  {
    ...
    entity.currentActivity match {
      ...
      case Idle =>
        // an simulation.entity can initiate an action, which will consume at least one tick, and
  ...
  def getOtherEntitiesRequired(action: PersonAction,
    ...
    action match {
      case simulation.actions.Farm =>
        val farmToUse = location.facilities.collectFirst {
          case f: simulation.facility.Farm if f.isAvailable => f
        }
        farmToUse match {
          case None => None
          case Some(farm) =>
  ...
  private def doEat(entity: Commoner): (Commoner, Mailbox) = {
    ...
    meal match {
      case None =>
        if (DEBUG) {
          println(s"No simulation.meal could be constructed from ${entity.inventory}")
        }
        (entity, Mailbox.empty)
      case Some(eatenMeal) =>
        ...
        if (DEBUG) println(f"Yum simulation.meal for ${entity.name} ${eatenMeal.kilocalories}%1.3f new fat $fatGained to reach $newBodyFat." +
          f"Remaining simulation.inventory has ${newInventory.totalAvailableCalories}%1.3f")
  ...
  private def doFarm(entity: Commoner): (Commoner, Mailbox) = {
    ...
    if (DEBUG) {
      ...
      println(s"old simulation.inventory ${entity.name} ${entity.inventory.contents.get(cropToFarm)}; new simulation.inventory ${newInventory.contents.get(cropToFarm)}")
    }
