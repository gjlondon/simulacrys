...
import simulation.convenienceTypes.ConvenienceTypes.VectorGrid
import simulation.facility.Facilities
import simulation.location._
...
class World private (startingGrid: Grid) {
  ...
  def printSummary(): Unit = {
    ...
    println(s"Total simulation.world population $totalPop")
  }
...
object Grid {
  ...
  def allFarms(startingTime: DateTime): Grid = {
    fillGrid {
      ...
      val facilities = Facilities(simulation.facility.Farm())
